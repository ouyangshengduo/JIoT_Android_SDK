...
public class MqttConnection implements MqttCallback {
    ...
    private static final int MQTT_PUBLISH_TIME_OUT = 3 * 1000;
    /**mqtt协议栈心跳时间间隔 300秒**/
    ...
    private SelfMqttActionListener selfMqttActionListener = new SelfMqttActionListener((MqttConstans.SUBSCRIBE));
    ...
    public synchronized void connect(@NonNull final MqttContext mqttContext, @NonNull final String mqttServer, @NonNull DeviceInfo deviceInfo,int protocolType){
        ...
        IMqttActionListener mActionListener = new IMqttActionListener() {
            ...
            public void onSuccess(IMqttToken token) {
                ...
                mqttHandleCallback.onConnectCompleted(Status.OK, token.getUserContext(), "connected to " + serverURI);
            }
            ...
            public void onFailure(IMqttToken token, Throwable exception) {
                ...
                if(exception.toString().contains(EXCEPTION_TYPE_STRING)) {
                    mqttHandleCallback.onConnectCompleted(Status.TIME_OUT, token.getUserContext(), exception.toString());
                }else{
                    mqttHandleCallback.onConnectCompleted(Status.ERROR, token.getUserContext(), exception.toString());
                }
            }
        };
    ...
    public void subscribe(@NonNull MqttContext mqttContext, @NonNull final String []topics) {
        ...
        if ((mqttClient != null) && (mqttClient.isConnected())) {
            try {
                mqttClient.subscribe(topics[0],1,mqttContext, selfMqttActionListener);
                //mqttClient.subscribe(topics[1],1,mqttContext, new SelfMqttActionListener((MqttConstans.SUBSCRIBE)));
