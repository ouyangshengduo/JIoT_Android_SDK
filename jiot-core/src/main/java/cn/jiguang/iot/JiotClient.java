...
import cn.jiguang.iot.bean.OtaUpgradeInform;
...
public class JiotClient implements JiotClientApi {
        ...
        if(null == executorService){
            ...
            ThreadFactory threadFactory = new ThreadFactory() {
                ...
                public Thread newThread(@NonNull Runnable runnable) {
                    return new Thread(runnable,"JIoT Client Action Thread");
                }
            };
    ...
    public void jiotDisConn() {
        ...
        MqttContext mqttContext = new MqttContext(JiotConstant.AUTO_DISCONNECT_CONTEXT, System.currentTimeMillis());
        disConn(mqttContext);
    }


    private void disConn(final MqttContext mqttContext){
    ...
    public void jiotRelease() {
        ...
        MqttContext mqttContext = new MqttContext(JiotConstant.RELEASE_JCLIENT_DISCONNECT_CONTEXT, System.currentTimeMillis());
        hadReleased = true;
    ...
    public JiotResult jiotPropertyReportReq(@NonNull PropertyReportReq properyReport) {
        ...
        for(int i = 0 ; i < properyReport.getProperties().length ; i ++){
            try {
                ...
                if(properyReport.getProperties()[i].getName() == null
                        || properyReport.getProperties()[i].getValue() == null){
                ...
                if(properyReport.getProperties()[i].getValue().isEmpty()
                        || properyReport.getProperties()[i].getValue().length() > 1024){
    ...
    public JiotResult jiotEventReportReq(@NonNull EventReportReq eventReportReq) {
        ...
        if(eventReportReq.getEvent().getContent().isEmpty()
    ...
    public JiotResult jiotVersionReportReq(VersionReportReq versionReportReq) {
        ...
        if(null == versionReportReq.getVersion()){
            return new JiotResult(JiotCode.JIOT_ERR_VERSION_APP_ERROR,versionReportReq.getSeqNo());
        }

        if(versionReportReq.getVersion().isEmpty()
        ...
        try {
            ...
            data.put("sdk_ver",JiotConstant.SDK_VERSION);
             ...
             // 2019-07-19 10:01
             // ouyangshengduo
            data.put("platform",JiotConstant.PLARTFORM);
            // MQTT消息
            MqttMessage message = new MqttMessage();
    ...
    @Override
    public JiotResult jiotOtaStatusReportReq(@NonNull OtaStatusReportReq otaStatusReportReq) {
        JiotLogger.d(JiotConstant.OTA_STATUS_REPORT_LOG);

        if(clientStatus.equals(CLIENT_STATUS.NONE)){
            JiotLogger.e("invalid disconnect action,client connect status error");
            return new JiotResult(JiotCode.JIOT_ERR_MQTT_STATE_ERROR, otaStatusReportReq.getSeqNo());
        }
        ...
        // 校验参数基本格式
        if(otaStatusReportReq.getSeqNo() < 0){
            //seqNO参数异常
            return new JiotResult(JiotCode.JIOT_ERR_SEQNO_ERROR, otaStatusReportReq.getSeqNo());
        }

        if(null == otaStatusReportReq.getDesc()){
            //参数异常
            return new JiotResult(JiotCode.JIOT_ERR_OTA_FORMAT_ERROR, otaStatusReportReq.getSeqNo());
        }

        if(otaStatusReportReq.getDesc().length() > 1024){
            //参数异常
            return new JiotResult(JiotCode.JIOT_ERR_OTA_DESC_FORMAT_ERROR, otaStatusReportReq.getSeqNo());
        }

        boolean isParamValid = (otaStatusReportReq.getStep() >= -6 && otaStatusReportReq.getStep() <= 104) || otaStatusReportReq.getStep() == 200;
        if(isParamValid){
            try {
                ...
                if(otaStatusReportReq.getSeqNo() == 0) {
                    ...
                    data.put("seq_no", otaStatusReportReq.getSeqNo());
                }

                JSONObject jsonData = new JSONObject();
                jsonData.put("task_id",otaStatusReportReq.getTaskId());
                jsonData.put("step",otaStatusReportReq.getStep());
                jsonData.put("desc",otaStatusReportReq.getDesc());
                ...
                // 用户上下文（请求实例）
                MqttContext mqttContext = new MqttContext("publishTopic", currentMills);
                mqttConnection.publish(mqttContext, String.format(MqttConstans.JMQTT_TOPIC_OTA_STATUS_REPORT_REQ, mqttConnection.getDeviceInfo().getProductKey(), mqttConnection.getDeviceInfo().getDeviceName()), message);
            }catch (Exception e){
                return new JiotResult(JiotCode.JIOT_ERR_OTA_DESC_FORMAT_ERROR, otaStatusReportReq.getSeqNo());
            }
        }else{
            //参数异常
            return new JiotResult(JiotCode.JIOT_ERR_OTA_STEP_VALUE_ERROR, otaStatusReportReq.getSeqNo());
        }
        return new JiotResult(JiotCode.JIOT_SUCCESS, otaStatusReportReq.getSeqNo());
    }
    ...
    private class SelfSisHandleCallBack extends AbstractSisHandleCallback {
        ...
        public void onResponse(String host, final String response) {
            ...
            if(autoDisconnect || hadReleased){
                JiotLogger.d("autoDisconnect is true or hadReleased is true.");
                return;
            }
            //拿到sis返回的mqtt服务器列表
            if(null == response){
            ...
            try {
                ...
                if(null != mqttConnection){
                    MqttContext mqttContext = new MqttContext(JiotConstant.CONNECT_FIRST_CONTEXT, System.currentTimeMillis());
    ...
    /**
     * 当连接broker失败后续处理，包括mqtt连接失败，mqtt订阅失败
     * @param status 失败的状态
     */
    private synchronized void serverConnectErrorAction(Status status,MqttContext mqttContext){
        //处理sis成功后，第一次连接mqtt服务器失败的处理
        if(mqttContext.getRequestType().equals(JiotConstant.CONNECT_FIRST_CONTEXT)){
            //判断是否需要更换mqtt服务端再连接
            if(mqttServers != null && mqttServers.length >= MAX_TRY_SIS_SERVERS_COUNT){
                if(isCanConn()) {
                    clientStatus = CLIENT_STATUS.CLIENT_RECONNECTING;
                    MqttContext mqttChangeIpAddressContext = new MqttContext(JiotConstant.CONNECT_CHANGE_IP_ADDRESS_CONTEXT, System.currentTimeMillis());
                    mqttConnection.connect(mqttChangeIpAddressContext, mqttServers[1], deviceInfo, protocolType);
                }
            }else{
                //无第二个有效IP mqtt服务器地址，直接走sis重连
                if (null != handleCallback) {
                    clientStatus = CLIENT_STATUS.CLIENT_DISCONNECTED;
        ...
        //更换IP地址连接mqtt服务器失败后的处理
        //同一个IP地址尝试重连mqtt服务器失败的处理
        else if(mqttContext.getRequestType().equals(JiotConstant.CONNECT_CHANGE_IP_ADDRESS_CONTEXT) || mqttContext.getRequestType().equals(JiotConstant.CONNECT_THE_SAME_IP_ADDRESS_CONTEXT)){
            if (null != handleCallback) {
                clientStatus = CLIENT_STATUS.CLIENT_DISCONNECTED;
                if(status.equals(Status.TIME_OUT)) {
                    handleCallback.jiotConnectFailHandle(JiotCode.JIOT_ERR_MQTT_NETWORK_CONNECT_ERROR);
    ...
    private class SelfMqttHandleCallback extends AbstractMqttHandleCallback {
        ...
        @Override
        public void onConnectCompleted(Status status, Object userContext, String msg) {
            ...
            String logInfo = String.format("onConnectCompleted, status[%s], userContext[%s], msg[%s]",
            ...
            if(status.equals(Status.OK)){
                ...
                //连接失败
                clientStatus = CLIENT_STATUS.CLIENT_DISCONNECTED;
                serverConnectErrorAction(status, (MqttContext) userContext);
            }
        }
        ...
        public void onConnectionLost(Throwable cause) {
            ...
            JiotLogger.d("onConnectionLost mqttConnection.getConnectStatus() " + mqttConnection.getConnectStatus());
            if(!mqttConnection.getConnectStatus().equals(MqttConstans.MQTT_CONNECT_STATUS.CONNECTED)
                    || !mqttConnection.getConnectStatus().equals(MqttConstans.MQTT_CONNECT_STATUS.CONNECTING)) {
                doConnAction(true);
            }
        }
        ...
        public void onDisconnectCompleted(Status status, Object userContext, String msg) {
            ...
            if(status.equals(Status.OK)){
                ...
                if(((MqttContext)userContext).getRequestType().equals(JiotConstant.SUBSCRIBE_ERROR_DISCONNECT_CONTEXT)){
                    //订阅错误导致的断连操作的后续处理
                    clientStatus = CLIENT_STATUS.CLIENT_DISCONNECTED;
                    if (null != handleCallback) {
                        handleCallback.jiotDisconnectHandle(JiotCode.JIOT_SUCCESS, "subscribe failed");
                    }
                    //判断是否需要更换mqtt服务端再连接
                    if (currentMqttServerIndex == 0 && !autoDisconnect) {
                        ...
                        if (null != mqttServers[currentMqttServerIndex] && null != mqttConnection) {
                            ...
                            if(isCanConn()) {
                                clientStatus = CLIENT_STATUS.CLIENT_RECONNECTING;
                                MqttContext mqttContext = new MqttContext(JiotConstant.CONNECT_CHANGE_IP_ADDRESS_CONTEXT, System.currentTimeMillis());
                                mqttConnection.connect(mqttContext, mqttServers[currentMqttServerIndex], deviceInfo, protocolType);
                                return;
                            }
                        }
                    ...
                    return;
                }else if(((MqttContext)userContext).getRequestType().equals(JiotConstant.PUBLISH_TIMEOUT_DISCONNECT_CONTEXT)){
                    //发布消息超时导致的断连操作的后续处理

                    if (null != handleCallback) {
                        handleCallback.jiotDisconnectHandle(JiotCode.JIOT_SUCCESS, "publish msg timeout");
                    }

                    if(!(clientStatus.equals(CLIENT_STATUS.CLIENT_CONNECTING) || clientStatus.equals(CLIENT_STATUS.CLIENT_RECONNECTING))) {
                        clientStatus = CLIENT_STATUS.CLIENT_DISCONNECTED;
                    }
                    ...
                    if (null != mqttServers[currentMqttServerIndex] && null != mqttConnection) {
                        if(isCanConn()) {
                            clientStatus = CLIENT_STATUS.CLIENT_RECONNECTING;
                            MqttContext mqttContext = new MqttContext(JiotConstant.CONNECT_THE_SAME_IP_ADDRESS_CONTEXT, System.currentTimeMillis());
                            mqttConnection.connect(mqttContext, mqttServers[currentMqttServerIndex], deviceInfo, protocolType);
                            return;
                        }
                    }
                ...
                }else if(((MqttContext)userContext).getRequestType().equals(JiotConstant.AUTO_DISCONNECT_CONTEXT)){
                    //用户主动断开连接
                    if(autoDisconnect && null != jiotAlarmHttpRetry){
                        jiotAlarmHttpRetry.stop();
                    }
                    clientStatus = CLIENT_STATUS.CLIENT_DISCONNECTED;
                    if (null != handleCallback) {
                    ...
                    return;
                }else if(((MqttContext)userContext).getRequestType().equals(JiotConstant.RELEASE_JCLIENT_DISCONNECT_CONTEXT)){
                    //用户调用release接口
                    if(autoDisconnect && null != jiotAlarmHttpRetry){
                        jiotAlarmHttpRetry.stop();
                    }
        ...
        public void onPublishCompleted(Status status, IMqttToken token, Object userContext, String errMsg) {
            ...
            if(status.equals(Status.TIME_OUT)){
                ...
                JiotLogger.d("onPublishCompleted mqtt status: " + mqttConnection.getConnectStatus());
                if(mqttConnection != null && mqttConnection.getConnectStatus().equals(MqttConstans.MQTT_CONNECT_STATUS.CONNECTED)){
                    initExecutorService().execute(new Runnable() {
                        @Override
                        public void run() {
                            MqttContext mqttContext = new MqttContext(JiotConstant.PUBLISH_TIMEOUT_DISCONNECT_CONTEXT,System.currentTimeMillis());
                            mqttConnection.disConnect(mqttContext);
                        }
                    });
                }
            ...
            if(status.equals(Status.OK)){
                ...
                if(mqttConnection != null && mqttConnection.getConnectStatus().equals(MqttConstans.MQTT_CONNECT_STATUS.CONNECTED)){
                    initExecutorService().execute(new Runnable() {
                        @Override
                        public void run() {
                            MqttContext mqttContext = new MqttContext(JiotConstant.SUBSCRIBE_ERROR_DISCONNECT_CONTEXT,System.currentTimeMillis());
                            mqttConnection.disConnect(mqttContext);
                        }
                    });
                }
        ...
        public void onMessageReceived(final String topic, final MqttMessage message) {
            ...
            }else if(topic.equals(String.format(MqttConstans.JMQTT_TOPIC_OTA_UPGRADE_INFORM_REQ,deviceInfo.getProductKey(),deviceInfo.getDeviceName()))){
                //OTA功能新增
                //客户端收到服务端下发的升级请求
                handleOTAUpgradeInformReceived(message);
            }else if(topic.equals(String.format(MqttConstans.JMQTT_TOPIC_OTA_STATUS_REPORT_RSP,deviceInfo.getProductKey(),deviceInfo.getDeviceName()))){
                //客户端上报升级状态后，客户端收到服务端的回复
                handleOTAStatusReportRspReceived(message);
            }
        }

        /**
         * 处理客户端上报升级状态后客户端收到服务端的回复
         * @param message 消息内容
         */
        private void handleOTAStatusReportRspReceived(MqttMessage message) {
            OtaStatusReportRsp otaStatusReportRsp = new OtaStatusReportRsp();
            int errorCode = 0;
            try {
                JSONObject jsonObject = new JSONObject(new String(message.getPayload()));
                otaStatusReportRsp.setSeqNo(jsonObject.getLong("seq_no"));
                otaStatusReportRsp.setCode(jsonObject.getInt("code"));
            } catch (Exception e) {
                e.printStackTrace();
                errorCode = JiotCode.JIOT_ERR_JCLI_JSON_PARSE_ERR;
            }
            if(null != messageCallback){
                messageCallback.jiotOTAStatusReportRsp(otaStatusReportRsp,errorCode);
            }
        }
         ...
         * @param message 消息内容
         */
        private void handleOTAUpgradeInformReceived(MqttMessage message) {
            OtaUpgradeInformReq otaUpgradeInformReq = new OtaUpgradeInformReq();
            int errorCode = 0;
            try {
                JSONObject jsonObject = new JSONObject(new String(message.getPayload()));
                otaUpgradeInformReq.setSeqNo(jsonObject.getLong("seq_no"));
                JSONObject otaUpgradeInformJson = new JSONObject(jsonObject.getString("data"));
                OtaUpgradeInform otaUpgradeInform = new OtaUpgradeInform();
                otaUpgradeInform.setTaskId(otaUpgradeInformJson.getLong("task_id"));
                otaUpgradeInform.setSize(otaUpgradeInformJson.getInt("size"));
                otaUpgradeInform.setUrl(otaUpgradeInformJson.getString("url"));
                otaUpgradeInform.setMd5(otaUpgradeInformJson.getString("md5"));
                otaUpgradeInform.setAppVersion(otaUpgradeInformJson.getString("app_ver"));
                otaUpgradeInformReq.setData(otaUpgradeInform);
                //  需要先进行回复确认
                otaUpgradeInformRsp(otaUpgradeInformReq.getSeqNo());
            } catch (JSONException e) {
                e.printStackTrace();
                errorCode = JiotCode.JIOT_ERR_JCLI_JSON_PARSE_ERR;
            }
            if(null != messageCallback){
                messageCallback.jiotOTAUpgradeInformReq(otaUpgradeInformReq,errorCode);
            }
        }
         ...
         * @param seqNo 消息序号
         */
        private void otaUpgradeInformRsp(long seqNo) {
            // MQTT消息
            MqttMessage message = new MqttMessage();
            JSONObject jsonObject = new JSONObject();
            try {
            ...
            mqttConnection.publish(mqttContext,String.format(MqttConstans.JMQTT_TOPIC_OTA_UPGRADE_INFORM_RSP,deviceInfo.getProductKey(),deviceInfo.getDeviceName()),message);
        }
    ...
    /**
     * 校验是否能够进行mqtt连接
     * @return false代表不能进行连接，true代表能进行连接
     */
    private boolean isCanConn() {
        JiotLogger.d("clientStatus is " + clientStatus);
        if(clientStatus.equals(CLIENT_STATUS.CLIENT_RECONNECTING) || clientStatus.equals(CLIENT_STATUS.CLIENT_CONNECTING)){
            JiotLogger.d("clientStatus is " + clientStatus);
            return false;
        }
        return true;
